...
const directorNoteFrFallback = `“Je ne sais pas si malgré les efforts déployés, la culture d'Okinawa survivra et si c'est le cas, comment elle évoluera. Avec UMUI, je souhaite rendre hommage à ces 'Gardiens des traditions' qui, à leur manière, s'efforcent de préserver et de maintenir vivante une culture menacée.”`
const directorNoteEnFallback = `“I do not know whether, despite all the efforts made, Okinawan culture will survive and if so, how it will evolve. With UMUI, I wish to pay tribute to these 'Guardians of Traditions' who, in their own way, strive to preserve and keep alive a threatened culture.”`
...
export default async function ConceptPage({ params: { locale } }: { params: { locale: Locale } }) {
  ...
  const showBlocksSanity = renderBlocks(conceptPage?.showDescription?.[locale])
  const directorNoteBlocksSanity = renderBlocks(conceptPage?.directorNote?.[locale])

  const filmText = filmBlocksSanity.length ? filmBlocksSanity : (locale === 'fr' ? filmTextFr : filmTextEn)
  const showText = showBlocksSanity.length ? showBlocksSanity : (locale === 'fr' ? showTextFr : showTextEn)
  const directorNoteText = directorNoteBlocksSanity.length
    ? directorNoteBlocksSanity.join(' ')
    : (locale === 'fr' ? directorNoteFrFallback : directorNoteEnFallback)
    ...
    <div className="max-w-4xl mx-auto px-6 py-24">
      ...
      <section className="mb-20 pt-16 border-t border-[#E8E4DE]">
        ...
        <div className="space-y-4 text-[#6B6B6B] leading-relaxed max-w-2xl">
          ...
          <p className="italic">{showItalic}</p>
        </div>
      </section>
